...
⚠️ 缩写歧义规则：
- "db" 在 K-pop/舞蹈/音乐语境中 = "dance break"（舞蹈间奏/舞蹈break段），非赌博，必判安全
- "db" 只有配合"下注""赔率""平台""稳赚""代理""包赔""输赢"等赌博上下文时才判违规
- 消息中出现舞蹈/音乐/K-pop/韩团/爱豆/challenge/翻跳相关话题时，"db"不判赌博
...
- K-pop/舞蹈/音乐术语："db"(dance break)、"导入"(intro)、"副歌"(chorus)、"challenge"(舞蹈挑战)、"翻跳"(cover dance) → ❌ 绝不判违规
